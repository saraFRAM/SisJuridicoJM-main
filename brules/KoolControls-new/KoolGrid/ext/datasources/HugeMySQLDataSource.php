...
//        print_r($_signs);
		...
		$_target = array("select"=>"","from"=>"","inner join"=>"","left join"=>"","right join"=>"","full join"=>"","where"=>"","group by"=>"","order by"=>"");
...
//        $_command_parts["order by"] = '';
//        $_command_parts["group by"] = '';
		
		$_count_command = join(" ",$_command_parts);
        //echo $_count_command."<br>";
...
//		echo $_select_command."<br>";
	...
	function GetAggregates($_arr) //Only with MYSQLDataSource
	{
		...
		foreach($_arr as $_aggregate)
		{
			...
			{
                $datafield = $_aggregate["DataField"];
                if ($_aggregate["DataField"]=='summePreis')
                    $datafield = 'preis';
				$_text .=  ", ".$_aggregate["Aggregate"]."(".$datafield.") as ".$_aggregate["Key"];				
			}
		}
