...
class OracleDataSource extends DataSource
{
	...
	function GetData($_start=0,$_count=999999)
	{
		$_tpl_select_command = "SELECT {fields} FROM ( SELECT a.*, rownum rnum FROM ({SelectCommand}) a {where} {orderby} {groupby} ) WHERE rownum <= ".($_start+$_count)." AND rnum > ".$_start;
