...
$tipoExp = (isset($_GET['tipoExp'])) ? $_GET['tipoExp'] : '';
$folder = obtFolderDigital($tipoExp);
$save_folder = 'upload/'.$folder.'/';
$destino = $save_folder.$date[0]."/".$date[1]."/";
$targetDir = $destino;
...
if (isset($_REQUEST["name"])) {
	...
	$fileName = $name."_".$_REQUEST["name"];
} elseif (!empty($_FILES)) {
	$fileName = $_FILES["file"]["name"];
} else {
	$fileName = $_FILES["file"]["name"]."_".uniqid("file_");
}
...
die('{"jsonrpc" : "2.0", "result" : null, "id" : "id", "fileParam":"'.$date[0]."/".$date[1]."/".$fileName.'"}');
